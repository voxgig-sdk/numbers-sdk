...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = NumbersOmni.make_runner(TEST_JSON_FILE, NumbersSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : NumbersOmni.errify(err)
    end
