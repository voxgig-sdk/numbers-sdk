...
The Ruby SDK for the Numbers API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.GetNumberFact(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.Numbers(nil).load(
  { "id" => "test01" }, nil
)
