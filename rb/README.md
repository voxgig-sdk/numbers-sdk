...
The Ruby SDK for the Numbers API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.GetNumberFact` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = NumbersSDK.test(testopts, sdkopts)
```
...
| `GetNumberFact` | `(data) -> GetNumberFactEntity` | Create a GetNumberFact entity instance. |
| `GetNumberTrivia` | `(data) -> GetNumberTriviaEntity` | Create a GetNumberTrivia entity instance. |
...
raise a `NumbersError` (a `StandardError` subclass), so wrap
...
```
rb/
├── Numbers_sdk.rb       -- Main SDK module
...
The main module (`Numbers_sdk`) exports the SDK class
