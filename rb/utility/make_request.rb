...
module NumbersUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = NumbersResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NumbersResponse.new(fetched)
    else
