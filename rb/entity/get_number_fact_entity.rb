...
class GetNumberFactEntity
  ...
  def make
    opts = @_entopts.dup
    GetNumberFactEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = NumbersHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetNumberFact, Hash] the current GetNumberFact data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = NumbersHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetNumberFact fields)
  def match_get
  ...
  #
  # @param reqmatch [GetNumberFactLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.GetNumberFact.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [GetNumberFact, Hash] the loaded GetNumberFact; raises NumbersError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = NumbersHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
