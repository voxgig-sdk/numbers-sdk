...
type Args struct {
	Entity string         `json:"entity" jsonschema:"get_number_fact | get_number_trivia | random"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.NumbersSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "numbers_list",
		Description: "List records from Numbers. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "numbers_load",
		Description: "Load a single record from Numbers. " +
...
func runOp(client *sdk.NumbersSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.NumbersSDK, name string) (sdk.NumbersEntity, error) {
	switch strings.ToLower(name) {
	case "get_number_fact":
		return client.GetNumberFact(nil), nil
	case "get_number_trivia":
		return client.GetNumberTrivia(nil), nil
	case "random":
