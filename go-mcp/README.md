...
[MCP](https://modelcontextprotocol.io) server exposing the Numbers SDK as
two agent tools — `numbers_list` and `numbers_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/numbers-mcp)
make build
...
claude mcp add --scope user numbers \
  -- /absolute/path/to/numbers-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./numbers-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// numbers_list: first page of records
{ "entity": "get_number_fact" }
{ "entity": "get_number_fact", "query": { } }

// numbers_load: one record by id
{ "entity": "get_number_fact", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/numbers-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user numbers \
     -- "$PWD"/dist/*/numbers-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `numbers_list` and `numbers_load` tools now appear
   in new sessions. Ask the agent to *"list get_number_fact using numbers"*
   and it calls `numbers_list` with `{"entity":"get_number_fact"}`.
...
```sh
export NUMBERS_APIKEY=sk_live_xxx            # API key
export NUMBERS_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `numbers_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `numbers_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NUMBERS_APIKEY` | API key sent with every request. |
| `NUMBERS_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

get_number_fact | get_number_trivia | random

### Smoke test via HTTP (raw JSON-RPC)

```sh
./numbers-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"numbers_load","arguments":{"entity":"get_number_fact","query":{"id":1}}}}'
```
