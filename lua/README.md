...
The Lua SDK for the Numbers API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:GetNumberFact(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Numbers(nil):load(
  { id = "test01" }, nil
)
