...
NumbersSDK._make_feature = _make_feature


function NumbersSDK.new(options)
  local self = setmetatable({}, NumbersSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function NumbersSDK:options_map()
...
function NumbersSDK:get_utility()
  return Utility.copy(self._utility)
end


function NumbersSDK:get_root_ctx()
  return self._rootctx
end


function NumbersSDK:prepare(fetchargs)
...
function NumbersSDK:direct(fetchargs)
...
-- by passing it.
function NumbersSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function NumbersSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "NumbersSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function NumbersSDK:GetNumberFact(data)
  ...
  if data == nil then
    if self._get_number_fact == nil then
      self._get_number_fact = EntityMod.new(self, nil)
    end
    return self._get_number_fact
  end
...
function NumbersSDK:GetNumberTrivia(data)
  ...
  if data == nil then
    if self._get_number_trivia == nil then
      self._get_number_trivia = EntityMod.new(self, nil)
    end
    return self._get_number_trivia
  end
  return EntityMod.new(self, data)
end
...
function NumbersSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = NumbersSDK.new(sdkopts)
  sdk.mode = "test"
