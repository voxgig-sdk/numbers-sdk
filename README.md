...
The Numbers API provides interesting facts about numbers covering trivia, math, dates, and years.

Learn more about Numbers API at [numbersapi.com/#random/math](http://numbersapi.com/#random/math).

This is an unofficial SDK for the Numbers public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — GetNumberFact, GetNumberTrivia and Random — that you
...
| --- | --- | --- |
| **GetNumberFact** | The GetNumberFact entity (load). | `/{number}/{type}` |
| **GetNumberTrivia** | The GetNumberTrivia entity (load). | `/{number}` |
| **Random** | The Random entity (load). | `/random/{type}` |
