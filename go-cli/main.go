// Command Numbers-cli is a CLI + REPL for the Numbers SDK,
...
const entitiesHelp = "get_number_fact get_number_trivia random"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      NUMBERS_APIKEY (api key), NUMBERS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: NUMBERS_APIKEY carries the API key and
	// NUMBERS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("NUMBERS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("NUMBERS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewNumbersSDK(opts)
