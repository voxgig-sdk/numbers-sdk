...
boru-driven command-line client **and** interactive REPL for the Numbers
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/numbers-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/numbers-cli load 1 get_number_fact
   ```
...
```sh
./numbers-cli load 1 get_number_fact          # scalar shorthand for {id:1}
./numbers-cli load '{id:1}' get_number_fact     # explicit match map
```
...
./numbers-cli load 1 get_number_fact
```
...
Run with no arguments to open a REPL (prompt `numbers>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_number_fact`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/numbers-cli`. |
...
(`./numbers-cli <expr>`) and interactively (the REPL), and expressions compose the
