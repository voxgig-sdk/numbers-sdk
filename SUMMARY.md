...
Results: Successful response with a number fact.
...
Results: Successful response with a number trivia fact.
...
Results: Successful response with a random number fact.
...
| --- | --- | --- | --- |
| GetNumberFact | `load` | `GET /{number}/{type}` | See reference |
| GetNumberTrivia | `load` | `GET /{number}` | See reference |
| Random | `load` | `GET /random/{type}` | See reference |

## Connect to the API

- Numbers API Server: `http://numbersapi.com`
...
- `numbers_list`: List records for an entity. No active entity supports this operation.
- `numbers_load`: Load one record for an entity. Supported entities: `get_number_fact`, `get_number_trivia`, `random`.
