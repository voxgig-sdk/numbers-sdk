...
class FtCtrl extends NumbersControl
{
...
class FtClient
{
    ...
    private NumbersUtility $utility;

    public function __construct(NumbersUtility $utility, array $options)
    {
    ...
    public function get_utility(): NumbersUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NumbersContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NumbersContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NumbersConfig::make_feature($name);
    ...
    private static function buildUrl(NumbersSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NumbersError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NumbersResponse($fetched);
        }
    ...
    private function fail(NumbersContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NumbersContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
