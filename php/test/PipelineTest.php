...
{
    private static function utility(): NumbersUtility
    {
    ...
    private static function ctx(array $over = []): NumbersContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new NumbersContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new NumbersOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof NumbersError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new NumbersOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = NumbersMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new NumbersOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = NumbersMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new NumbersOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = NumbersMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new NumbersSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = NumbersMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new NumbersResponse([]),
            'result' => new NumbersResult([]),
        ]);
        [, $err] = NumbersMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new NumbersSpec([]),
            'response' => null,
            'result' => new NumbersResult([]),
        ]);
        [, $err] = NumbersMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new NumbersSpec(['step' => 's']),
            'response' => new NumbersResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new NumbersResult([]),
        ]);
        [, $err] = NumbersMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new NumbersSpec(['step' => 's']),
            'response' => new NumbersResponse(self::resp(200, ['v' => 1])),
            'result' => new NumbersResult([]),
        ]);
        [, $err] = NumbersMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new NumbersSpec(['step' => 's']),
            'response' => new NumbersResponse(self::resp(200, ['v' => 2])),
            'result' => new NumbersResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        NumbersMakeResponse::call($ctx);
    ...
    {
        $preset = new NumbersResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new NumbersSpec([]),
            'response' => new NumbersResponse([]),
            'result' => new NumbersResult([]),
        ...
        [$got, $err] = NumbersMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new NumbersResult([])]);
        [, $err] = NumbersMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new NumbersSpec([]), 'result' => null]);
        [, $err] = NumbersMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new NumbersSpec(['step' => 's']),
            'result' => new NumbersResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new NumbersOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = NumbersMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new NumbersSpec(['step' => 's']),
            'result' => new NumbersResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new NumbersOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = NumbersMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new NumbersResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new NumbersSpec([]),
            'result' => new NumbersResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = NumbersMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = NumbersMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new NumbersError('boom', 'boom');
        $utility->fetcher = function (NumbersContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new NumbersSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = NumbersMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (NumbersContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = NumbersMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(NumbersResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (NumbersContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new NumbersSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = NumbersMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new NumbersResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new NumbersSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = NumbersMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = NumbersMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = NumbersMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new NumbersResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, NumbersDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new NumbersResult(['ok' => false])]);
        $this->expectException(NumbersError::class);
        NumbersDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new NumbersResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', NumbersMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        NumbersMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new NumbersBaseFeature();
        $b = new NumbersBaseFeature();
        NumbersFeatureAdd::call($ctx, $a);
        NumbersFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): NumbersBaseFeature
    {
        $f = new NumbersBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        NumbersFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): NumbersContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new NumbersSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = NumbersPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new NumbersResponse(['headers' => null]),
            'result' => new NumbersResult([]),
        ]);
        NumbersResultHeaders::call($ctx);
