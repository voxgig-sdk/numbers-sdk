...
// Numbers SDK GetNumberFact entity
...
class GetNumberFactEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GetNumberFactEntity($this->_client, $opts);
    }

    /**
     * @param GetNumberFact|array $args GetNumberFact data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = NumbersHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetNumberFact|array The current GetNumberFact data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetNumberFact fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = NumbersHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetNumberFact fields).
     */
    ...
    /**
     * Load a single GetNumberFact.
     *
     * @param GetNumberFactLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed GetNumberFactLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetNumberFact|array The loaded GetNumberFact as an assoc-array at the
     *   SDK boundary; throws NumbersError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = NumbersHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
