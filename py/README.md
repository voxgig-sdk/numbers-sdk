...
The Python SDK for the Numbers API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.GetNumberFact(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Numbers(None).load(
    {"id": "test01"}, None
)
...
```
py/
├── numbers_sdk.py         -- Main SDK module
