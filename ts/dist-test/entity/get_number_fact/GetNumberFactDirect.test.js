...
(0, node_test_1.describe)('GetNumberFactDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-get_number_fact', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-get_number_fact', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["number01", "type01"]))
            return;
...
function liveScenariosActive() { return false && process.env.NUMBERS_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.NumbersSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['NUMBERS_TEST_GET_NUMBER_FACT_ENTID'];
    ...
    const client = new __1.NumbersSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=GetNumberFactDirect.test.js.map