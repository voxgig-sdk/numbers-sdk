...
//     (\`new NumbersSDK(...)\` / \`NumbersSDK.test(...)\` become
//     \`NumbersSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"get_number_fact":{"test01":{"id":"test01"}},"get_number_trivia":{"test01":{"id":"test01"}},"random":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new NumbersSDK(...)\` and any
// \`NumbersSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+NumbersSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/NumbersSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
