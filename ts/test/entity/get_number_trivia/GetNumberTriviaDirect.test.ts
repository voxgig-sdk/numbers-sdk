...
describe('GetNumberTriviaDirect', async () => {
  ...
  test('direct-load-get_number_trivia', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-get_number_trivia', setup.live)) return
