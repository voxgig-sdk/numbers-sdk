...
describe('GetNumberFactDirect', async () => {
  ...
  test('direct-load-get_number_fact', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-get_number_fact', setup.live)) return
    if (skipIfMissingIds(t, setup, ["number01","type01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['NUMBERS_TEST_GET_NUMBER_FACT_ENTID']
