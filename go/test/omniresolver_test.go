...
import (
	...
	omni "github.com/voxgig-sdk/numbers-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.NumbersError); is {
				out := map[string]any{
					"name":    "NumbersError",
...
func omniCtx(arg any, client *sdk.NumbersSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.NumbersSDK); is {
			client = live
